...
AccessorFunc(PANEL, "bAutoScroll", "AutoScroll")
AccessorFunc(PANEL, "bPreferBottom", "PreferBottom")
...
function PANEL:Init()
	...
	self.ScrollUp = self:Add("afterglow_scrollbutton")
	self.ScrollUp:SetPos(self:GetWide() - 15, 0)
	self.ScrollUp:SetDirection(1)

	self.ScrollDown = self:Add("afterglow_scrollbutton")
	self.ScrollDown:SetPos(self:GetWide() - 15, 0)
	self.ScrollDown:SetDirection(-1)

	self.ScrollGrip = self:Add("afterglow_scrollgrip")
...
function PANEL:ResizeCanvas(height, force)
	...
	if self.bAutoScroll and (self:IsAtBottom() or force) then
...
function PANEL:MoveCanvas(delta)
	local x, y, _, h = self.Canvas:GetBounds()
...
function PANEL:OnMouseWheeled(delta)
	...
	self:MoveCanvas(delta * self.fScrollAmount)
end

function PANEL:GetBottom()
	if self.bPreferBottom then
...
derma.DefineControl("afterglow_scrollpanel", "", PANEL, "EditablePanel")
...
derma.DefineControl("afterglow_scrollgrip", "", PANEL, "EditablePanel")
...
derma.DefineControl("afterglow_scrollbutton", "", PANEL, "DButton")
